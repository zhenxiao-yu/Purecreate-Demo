...
2. In Vercel: **New Project** → import the repo → set the **Root Directory** to `next/`.
3. Add `OPENAI_API_KEY` as an environment variable (Production + Preview).
4. Deploy. The `/api/image` route runs as a Node.js serverless function (set to `maxDuration = 60` to accommodate DALL·E latency).
...
| Old | New |
|---|---|
| `ui/` Vite + React 18 | `next/src/` Next 15 App Router |
| `server/` Express + dotenv | `next/src/app/api/*/route.js` |
